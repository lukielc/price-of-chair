...
class User(object):
    ...
    def register_user(email, password):
        ...
        The password already comes hashed as sh-512.
        :param email: user's email (might be invalid)
        :param password: sha512-hashed password
        :return: True if registered successfully, or False otherwise (exceptions cab also be raised)
        """

        user_data = Database.find_one(UserConstants.COLLECTION, {"email": email})

        if user_data is not None:
            raise UserErrors.UserAlreadyRegisteredError("This email already exist.")
